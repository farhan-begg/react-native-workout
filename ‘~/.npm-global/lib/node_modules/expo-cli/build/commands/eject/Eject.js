...
async function configureIOSStepAsync(projectRoot) {
  ...
  if (_configPlugins().WarningAggregator.hasWarningsIOS()) {
...
async function configureAndroidStepAsync(projectRoot) {
  ...
  if (_configPlugins().WarningAggregator.hasWarningsAndroid()) {
...
async function warnIfDependenciesRequireAdditionalSetupAsync(pkg, options) {
  const expoPackagesWithExtraSetup = ['expo-camera', 'expo-image-picker', 'expo-av', 'expo-background-fetch', 'expo-barcode-scanner', 'expo-brightness', 'expo-calendar', 'expo-contacts', 'expo-file-system', 'expo-location', 'expo-media-library', 'expo-notifications', 'expo-screen-orientation', 'expo-sensors', 'expo-task-manager'].reduce((prev, curr) => ({ ...prev,
